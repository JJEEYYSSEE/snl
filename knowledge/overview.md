...
them on **single-use sabotage snakes** that knock opponents back and steal their
points. Using the economy well is the whole game — just rolling loses.
...
Prompted (or via `--players/--humans/--difficulty`):
1. Players total **2-4**
2. Human players **0-N** (rest are AI)
3. AI difficulty (easy/hard) — only if any AI
...
- **Ladders:** climb on exact landing (also on board entry).
- **Snakes:**
  - *Board snakes* = fixed terrain, bite on exact head only.
  - *Player snakes* = single-use traps with a **strike range** (head + 2 below);
    bite reliably when well placed; consumed on fire (re-placeable). **Owner
    immune to own snakes.** Jumping clean over is safe.
  - A bite **slides you back + steals points** to the owner.
- **Economy:** scarce tile income (~4-14/turn); snake cost sub-linear
  (`2 × purchase_count × length^0.9`, min 12) — save up for big knockbacks.
...
- **Hard = PPO**, 4-action strategy space (roll / cheap trap / save-for-big /
  win-denial lurk) with catch-optimal placement. **Beats Easy ~94%** (300-game
  eval). Falls back to Expectimax if the model can't load.
